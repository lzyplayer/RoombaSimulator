...
public class Room {
	...
	public Point2D getStartPos(int radius) {
		Random ran_s = new Random();
		int clear_x = ran_s.nextInt(width-radius*2)+radius*2;
		int clear_y = ran_s.nextInt(height-radius*2)+radius*2;
		while (getData(clear_x,clear_y) == RoomTile.BLOCKED){
			clear_x = ran_s.nextInt(width);
			clear_y = ran_s.nextInt(height);
		}
		return new Point2D(clear_x,clear_y);
	}
